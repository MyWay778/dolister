...
mongoose.connect("mongodb+srv://ivan:ivan@cluster0.cox6i.mongodb.net/bwa_test?retryWrites=true&w=majority", {useNewUrlParser: true, useUnifiedTopology: true})
...
app.post("/user", (req, res) => {
    if (req.body.action === "signup") {
        User.find({}, (err, users) => {
            ...
            if (users.length > 20) {
                res.json({"action": "signup", "message": "Sorry max twenty users"})
            } else {
                ...
                res.status(200).json({"action": "signup", "message": result.username+" successfully registered"})
                })
            }
        })
...
app.post("/todos/:action", (req, res) => {
    switch(req.params.action) {
        ...
        case "done": 
            ToDo.updateOne({"_id":req.body.id }, {"completed": req.body.completed}, (err, task) => {
                if (err) {
                    console.log(err)
                    res.sendStatus(500)
                }
        ...
        case "edit": 
            ToDo.updateOne({"_id":req.body.id}, {"description": req.body.description}, (err, task) => {
                if (err) {
                    console.log(err)
                    res.sendStatus(500)
                }
